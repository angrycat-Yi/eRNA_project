...
def getparser():
	...
	parser.add_argument("-int","--inter",required = False, default=50)
	parser.add_argument("-o", "--out", required = True, help="output the bed file")
	args = parser.parse_args()
	return args


def change_bed(bed,out,value):
		...
		with open(bed,"r") as f:
			for lines in f:
				...
				if re.match('[A-Za-z]+',lst[1]) or re.match('[A-Za-z]+',lst[2]):
					...
					start = int(lst[1]) + len//2 - int(value)
					end = int(lst[1]) + len//2 + int(value)
...
if __name__ == "__main__":
	...
	args = getparser()
	bed = args.bed	
	change_bed(bed,args.out,args.inter)	
